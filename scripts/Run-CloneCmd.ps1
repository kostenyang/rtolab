<#
.SYNOPSIS
    Run an ad-hoc /bin/sh command in a nested ESXi clone via GuestOps and return output.
#>
[CmdletBinding()]
param(
...
# Wrap command in a script file so all stdout/stderr redirects via top-level redirection
...
$spec = New-Object VMware.Vim.GuestProgramSpec -Property @{
    ProgramPath='/bin/sh'; Arguments='-c "chmod +x /tmp/runcmd.sh && /bin/sh /tmp/runcmd.sh > /tmp/runcmd.out 2>&1"'; WorkingDirectory='/tmp'
}
$gpid = $pm.StartProgramInGuest($mo, $auth, $spec)
for ($i = 0; $i -lt 60; $i++) {
    $p = $pm.ListProcessesInGuest($mo, $auth, @($gpid))
